...
def main_script(connection, cursor, queries):
    ...
    if main_choice == "0":
        ...
        print(
            DBManager.execute_user_action(user_choice, table_choice, cursor, connection)
        )
    connection.commit()
    connection.close()
...
def main():
    while True:
        try:
            ...
            answer = main_script(connection, cursor, queries)
            if answer == "exit":
                break
